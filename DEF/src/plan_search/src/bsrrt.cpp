...
int RSAstar::search(Eigen::Matrix<double, 3, 3> init_control, Eigen::Vector3d end_pt,double time_start)
{
  ...
  // std::cout<<start_node->position<<std::endl;
  ...
  { 
    ...
    openList_.pop();
            std::priority_queue<PathNodePtr, std::vector<PathNodePtr>, NodeComparator> empty_queue;
        openList_.swap(empty_queue);
    // Eigen::Vector3i index1 = currentPtr->index;
    // currentPtr = BSplineGridNodeMap[index1(0)][index1(1)][index1(2)];

    if(currentPtr->node_state == IN_CLOSE_SET)
    {
      continue;
    }
    // std::cout << "Openlist size: " << expanded_nodes_.openlist.size() << std::endl;
    // std::cout << "closelist size: " << expanded_nodes_.closelist.size() << std::endl;
    ...
    count = count + 1;
    // std::cout<<"currentPtr->pt:"<<currentPtr->position.transpose()<<std::endl; 
    std::cout<<"currentPtr->in:"<<currentPtr->index.transpose()<<std::endl; 
    // if(count ==3)
    //   return 0;
    // expanded_nodes_.open2closelist(currentPtr->index,currentPtr);
    currentPtr->node_state = IN_CLOSE_SET;

    // std::cout<<"openList_:"<<openList_.size()<<std::endl;
    // std::cout<<"timer_astar.ElapsedMs():"<<timer_astar.ElapsedMs()<<std::endl;
    // std::cout<<"timer_astar.ElapsedMs():"<<timer_astar.ElapsedMs()<<std::endl;
