...
class RSAstar {
 private:
  ...
  std::vector<double> d; //这个值真的很重要的，最后求解的向量
  // double tentative_gScore;
  double Time_Obstacle;


  double max_vel_,max_acc_,horizon_,tie_breaker;
  double ts;
  double time_start;
  double max_runtime_;
 ...
 public:
  ...
  ~RSAstar();
    /* helper */
  std::vector<Eigen::Vector3d> getctrlpoint();
  inline void setQuadraticCostWeight(std::array<double, P_ +1> weight_array) {quadratic_cost_weight = weight_array;};
  ...
  void   Neighbor(PathNodePtr current,std::vector<PathNodePtr>& neighbors); 
  bool   checkTrajDynamics(PathNodePtr neighbor);
  bool CheckStaticObstacle(PathNodePtr neighbor,double num);
