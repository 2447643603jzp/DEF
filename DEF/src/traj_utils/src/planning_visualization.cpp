...
PlanningVisualization::PlanningVisualization(ros::NodeHandle& nh) {
  ...
  traj_pub_1 = node.advertise<visualization_msgs::Marker>("/planning_vis/trajectory1", 20);
  pubs_.push_back(traj_pub_1);
...
void PlanningVisualization::drawBspline(UniformBspline& bspline, double size,
                                        ...
                                        const Eigen::Vector4d& color2, int id1, int id2,int id3) {
  ...
  displaySphereList(traj_pts, size, color, BSPLINE + id1 % 100,id3);
  ...
  displaySphereList(ctp, size2, color2, BSPLINE_CTRL_PT + id2 % 100,id3);
}
