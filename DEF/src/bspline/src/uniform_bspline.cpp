...
namespace fast_planner
{
                                         ...
                                         const double &interval)
  {
    ...
    for (int i = 0; i <= m_; ++i)
    {
      ...
      if (i <= p_)
      {
        u_(i) = double(-p_ + i) * interval_; // 把前面一段时间设置成复数了
      }
  ...
  Eigen::MatrixXd UniformBspline::getDerivativeControlPoints()
  {
    ...
    // std::cout<<"control_points_.rows()"<<control_points_.rows()<<std::endl;
    // std::cout<<"control_points_.cols()"<<control_points_.cols()<<std::endl;
    for (int i = 0; i < ctp.cols(); ++i)
    {
      ctp.col(i) =
          p_ * (control_points_.col(i + 1) - control_points_.col(i)) / (u_(i + p_ + 1) - u_(i + 1)); ///得到控制点的导数
    }
    return ctp;
  }
  ...
  {
    // std::cout<<"aaaaaaa2"<<std::endl;
    Eigen::MatrixXd ctp = getDerivativeControlPoints();
    // std::cout<<"aaaaaaa1"<<std::endl;
