...
struct MappingData {
  ...
  cv::Mat depth_image_, rgb_image_, last_depth_image_;
  int image_cnt_;
  ...
  bool occ_need_update_,detect_need_update_,clear_dynamic, local_updated_, esdf_need_update_;
  bool rgb_get;
...
public:
  ...
  inline void getMinMaxId(Eigen::Vector3i &min_id,Eigen::Vector3i &max_id);
  ...
  int getVoxelNum();
  void setDetect(BoxDetector::Ptr detect)
  {
    this->detector_ = detect;
  }
  void rgbCallback(const sensor_msgs::ImageConstPtr& img_rgb ) ;
...
private:
  ...
  void depthPoseCallback(const sensor_msgs::ImageConstPtr& img,const sensor_msgs::ImageConstPtr& img_rgb,
                         const geometry_msgs::PoseStampedConstPtr& pose);
  void depthOdomCallback(const sensor_msgs::ImageConstPtr& img,const sensor_msgs::ImageConstPtr& img_rgb,
  ...
  void detectCallback(const ros::TimerEvent& /*event*/);
  ...
  shared_ptr<message_filters::Subscriber<sensor_msgs::Image>> rgb_sub_;
  ...
  ros::Timer occ_timer_, esdf_timer_, vis_timer_,detect_timer;
...
inline void SDFMap::getMinMaxId(Eigen::Vector3i &min_id,Eigen::Vector3i &max_id)
{
  min_id = min_ID;
  max_id = max_ID;
}
