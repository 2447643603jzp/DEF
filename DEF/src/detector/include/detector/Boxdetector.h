...
public:
  ...
  void addhandhistory(double time)
  {
    cluster c = getLastCluster();
    double c_time = c.time;
    c.time = time;
    std::pair<double,double> v_ = getV();
    //  std::cout<<"position:"<<v_.first<<" "<<v_.second<<std::endl;
    // std::cout<<"331266k"<<std::endl;
    Eigen::Vector3d position;
    position = c.centroid + Eigen::Vector3d(v_.first*(time-c_time),v_.second*(time-c_time),0);
  ...
  {
    if(t-time_traj_keeping>max_seconds_keeping_traj)
    {
      return true;
    }
    else{
  ...
  { 
    ...
    std::vector<Eigen::Vector3d> positions;
    std::pair<double,double> v_ = getV();
    // std::cout<<"v:"<<v_.first<<" "<<v_.second<<std::endl;
    Eigen::Vector3d pt = getLatestposition();
    positions.push_back(getLatestposition()); // last position
    double time_now = getLatestTime();
    double time_end = time_now + duration;
    for (double t= time_now;t<time_end;t = t+0.3)
    {
  ...
  void kfEstimate(cluster &b) {
    ...
    // std::cout<<"b.vx:"<<states(2,0)<<std::endl;
    // std::cout<<"b.vy:"<<states(3,0)<<std::endl;
    b.vx = abs(states(2,0))>0.15?states(2,0):0;
    b.vy = abs(states(3,0))>0.15?states(3,0):0;
    // std::cout<<"b.vx:"<<b.vx<<" "<<"b.vy:"<<b.vy<<std::endl;
    // std::cout<<"x:"<<states(0,0)<<" "<<"b.y:"<<states(1,0)<<std::endl;
    // std::cout<<" "<<std::endl;
  ...
  { 
    ...
    // std::cout<<"eval3"<<std::endl;
    // std::cout<<"num:"<<int(double(num)*2/3)<<std::endl;
    if(flag > int(double(num)*2/3))
    {
      return true;
    }
    else
...
private:
  ...
  double max_seconds_keeping_traj = 0.8;   // 轨迹保留的最多的时间。 // 这一条轨迹值不值得，已经进行了合理的检测。
  polynomial traj;
...
class BoxDetector
{
  ...
  void StoreAllTrack(double time_st,double ts,std::vector<std::vector<Eigen::Vector3d>> &positionss);
  std::vector<std::vector<double>> getboxes();
  ConvexHullsOfCurves_Std gettemphull(int q_segment);
  bool checkReplan(Eigen::Vector3d pose);
  int getallnumtrack();
  bool checkReplan1(Eigen::Matrix<double, 4, 3>  Q);
